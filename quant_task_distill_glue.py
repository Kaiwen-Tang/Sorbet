...
def main():
    ...
    parser.add_argument('--sym_quant_ffn_attn', action='store_true',
                        help='whether use sym quant for attn score and ffn after act') # default asym -- unsigned
    parser.add_argument('--sym_quant_qkvo', action='store_true',  default=True,
                        help='whether use asym quant for Q/K/V and others.') # default sym -- signed
    ...
    if not args.do_eval: # need the teacher model for training
        # print("teacher model input", config.input_bits)
        if hasattr(config, "input_bits") and config.input_bits < 32:
            teacher_model = QuantBertForSequenceClassificationT.from_pretrained(args.teacher_model, config=config)
        else:
    ...
    if args.do_eval:
        student_model.eval()
        learner._do_eval(student_model, task_name, eval_dataloader, output_mode, eval_labels, num_labels) 
    else:
