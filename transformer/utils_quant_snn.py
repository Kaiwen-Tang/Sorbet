...
def create_spike_input_cuda(input,T, theta):
    ...
    out = catCuda.getSpikes(out, 0.95 * (theta-0.0001))
    return out
...
class QuantizeLinear(nn.Linear):
    ...
    def forward(self, input):
        ...
        input, alpha, bias = act_quant_fn(input, self.input_clip_val, num_bits=self.input_bits, symmetric=self.symmetric,
                             quant_method=self.input_quant_method, layerwise=self.input_layerwise)
        # rate = torch.mean(input)
        # file_path = 'spike_rate.txt'
        # with open(file_path, 'a') as f:
        #     f.write(f'{rate.item()}\n')
        ...
        if self.symmetric:
            if input.dim() == 4:
                ...
                out = torch.sum(out1.permute(0,2,3,1), dim=-1) * alpha
            else:
                # input = torch.sum(input, dim=-1)
                # input = (input - 8) * alpha
                input = (input-0.5).permute(0,2,1)
                out = (input*alpha).matmul(weight.t())
                out = torch.sum(out.permute(0,2,1), dim=-1)

            # bias = torch.full(tuple(input.shape), 0.5).cuda()
            # out2 = (bias * alpha).matmul(weight.t())
            # out = out1 - out2
