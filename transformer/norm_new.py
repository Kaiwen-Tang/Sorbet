...
def LUT_power2(tensor):
    tensor = torch.where(tensor <= 0.25, 0.25, tensor)
    tensor = torch.where((tensor > 0.25) & (tensor <= 0.5), 0.5, tensor)
    tensor = torch.where((tensor > 0.5) & (tensor <= 1), 1.0, tensor)
    tensor = torch.where((tensor > 1) & (tensor <= 2), 2.0, tensor)
    tensor = torch.where((tensor > 2) & (tensor <= 4), 4.0, tensor)
    tensor = torch.where((tensor > 4) & (tensor <= 8), 8.0, tensor)
    tensor = torch.where((tensor > 8) & (tensor <= 16), 16.0, tensor)
    return tensor
...
class GroupScaling1D(nn.Module):
    ...
    def forward(self, input):
        ...
        abs_input = torch.abs(gn_input)
        moment1 = torch.repeat_interleave(torch.mean(abs_input, dim=3, keepdim=True), 
                                   repeats=Cg, dim=-1)
        moment1 = moment1.contiguous().reshape(T, B, C)

        # LUT_power2
        # moment1 = LUT_power2(moment1 + self.eps)
        # print(moment1 + self.eps)
        moment1 = find_closest_power_of_two(moment1 + self.eps)
        # print(torch.min(moment1), torch.max(moment1))
        return input / moment1
...
class PowerFunction(torch.autograd.Function):
    ...
    def forward(ctx, x, weight, bias, running_phi, eps, afwd, abkw, ema_gz, \
        ...
        weight = torch.sign(weight) * 2 ** torch.round(torch.log2(torch.abs(weight)))
...
class MaskPowerNorm(nn.Module):
    ...
    def forward(self, input, pad_mask=None, is_encoder=False):
        ...
        if self.training:
            ...
            N, C, H, W = input.size()
            # print(input.shape) 64,512,1,1
            var = self.running_phi
            # print(var.shape) 1,512,1,1
            output = input # / (var + self.eps).sqrt()
            # print(self.eps.shape) value: 1e-5
            # print(output.shape) 64,512,1,1
            # print(self.weight.shape) 512
            # print(self.weight.reshape(1,C,1,1).shape) 1,512,1,1
            weight = self.weight.reshape(1,C,1,1) / ((var + self.eps).sqrt())
            weight = torch.sign(weight) * 2 ** torch.round(torch.log2(torch.abs(weight)))
            output = weight * output + self.bias.reshape(1,C,1,1)
        ...
        output[negative_mask] = - 2 ** torch.round(torch.log2(torch.abs(output[negative_mask])))
        positive_mask = output > 0
        output[positive_mask] = 2 ** torch.round(torch.log2(torch.abs(output[positive_mask])))
